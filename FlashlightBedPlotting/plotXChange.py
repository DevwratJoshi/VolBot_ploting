...
if len(sys.argv) < 4:
    print("Usage: plotXChange.py {R/B/A (The parameter to change)} {value of second param in order RBA} {value of third param in RBA}")
    sys.exit()
big = 60*2
small = 20*2
mid = 30*2
...
if(sys.argv[1] == 'R'):
    ...
    for i in range(len(list1)):
        for d in data_list:
            url = "/home/dev/Devwrat/Masters/Research/Journal papers/AMAM special issue/data/VolBot_Flashlight_Bed_sims/data_R_alpha_beta/R" + list1[i] + "/Beta" + beta + "/alpha_fold" + alpha + "/data" + d + "/Flash_" + list1[i] + ".0_" + beta + "_" + alpha
            data = pd.read_csv(url, header=None, index_col=False)
            xPosInit[0] = xPosInit[0] + data.values[0][0]
            #print(np.median(data.values[-101:-1,0]))
            xPosSdev.append(np.median(data.values[-101:-1,0]))
            #xPosSdev.append(sum(data.values[-101:-1,0])/len(data.values[-101:-1,0]))
    ...
    for i in range(len(list1)):
        for d in data_list:
            url = "/home/dev/Devwrat/Masters/Research/Journal papers/AMAM special issue/data/VolBot_Flashlight_Bed_sims/data_R_alpha_beta/R" + R + "/Beta" + list1[i] + "/alpha_fold" + alpha + "/data" + d + "/Flash_" + R + ".0_" + list1[i] + "_" + alpha
            data = pd.read_csv(url, header=None, index_col=False)
            xPosInit[0] = xPosInit[0] + data.values[0][0]
            #print(np.median(data.values[-101:-1,0]))
            xPosSdev.append(np.median(data.values[-101:-1,0]))
    ...
    for i in range(len(list1)):
        for d in data_list:
            url = "/home/dev/Devwrat/Masters/Research/Journal papers/AMAM special issue/data/VolBot_Flashlight_Bed_sims/data_R_alpha_beta/R" + R + "/Beta" + beta + "/alpha_fold" + list1[i] + "/data" + d + "/Flash_" + R + ".0_" + beta + "_" + list1[i]
            data = pd.read_csv(url, header=None, index_col=False)
            xPosInit[0] = xPosInit[0] + data.values[0][0]
            #print(np.median(data.values[-101:-1,0]))
            xPosSdev.append(np.median(data.values[-101:-1,0]))
