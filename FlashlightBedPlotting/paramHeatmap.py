...
small = 20*2
mid = 30*2
R_list = ['90', '150', '210','270'] #The length of the flashlight in mid module lengths
...
if 'A' in sys.argv and 'B' in sys.argv:
    ...
    for i in range(len(list2)):
        for j in range(len(list1)):
            for d in data_list:
                url = "/home/dev/Devwrat/Masters/Research/Journal papers/AMAM special issue/data/VolBot_Flashlight_Bed_sims/"+flashSize+"_flashlight/" + "Freq" + freq + "Amp" + amp + "/" + "data_R_alpha_beta/R" + R  + "/Beta" + list2[i] + "/alpha_fold" + list1[j] + "/data" + d + "/Flash_" + R  + ".0_" + list2[i] + "_" + list1[j]
    ...
    for i in range(len(list2)):
        for j in range(len(list1)):
            for d in data_list:
                url = "/home/dev/Devwrat/Masters/Research/Journal papers/AMAM special issue/data/VolBot_Flashlight_Bed_sims/"+ flashSize + "_flashlight"+"Freq" + freq + "Amp" + amp + "/" + "data_R_alpha_beta/R" + list2[i]  + "/Beta" + beta + "/alpha_fold" + list1[j] + "/data" + d + "/Flash_" + list2[i]  + ".0_" + beta + "_" + list1[j]
                data = pd.read_csv(url, header=None, index_col=False)
                if not coord:
                    xPosInit[0] = xPosInit[0] + data.values[0][0]
                    xPosInit[1:] = xPosInit[1:] + data.values[-101:-1, 0]
